...
MetableLog::MetableLog(const char *file_name, int line_number,
                       MetableLogLevel severity)
    : filename_(file_name), line_number_(line_number),
  ...
  if (logging_provider == nullptr) {
    try {
      logging_provider = ::spdlog::rotating_logger_mt(
          "Metable_log", log_file_name,
          1024 * 1024 * 1024 * log_file_roll_size_mb, log_file_roll_cout);
